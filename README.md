...
1. You export photos from Lightroom Classic and upload them to a Cloudflare R2 bucket using Cyberduck (or any S3-compatible client).
...
│                                                             │
│  Lightroom Classic                                          │
│       │  export JPEG (3000px, quality 90, Display P3,       │
│       │              output sharpening: Screen High)        │
│       ▼                                                     │
│  Cyberduck (or any S3-compatible client)                    │
...
Full example with current categories:
...
**Recommended macOS client:** [Cyberduck](https://cyberduck.io) (free)

Cyberduck has a built-in Cloudflare R2 connection profile. Download the profile from [docs.cyberduck.io/protocols/s3/cloudflare](https://docs.cyberduck.io/protocols/s3/cloudflare/) and open it to pre-configure the connection type. You'll need your R2 Access Key ID, Secret Access Key, and Account ID.

Note: Transmit does not currently support R2 (R2 lacks the chunked transfer encoding Transmit requires).

### Per-shoot workflow — album-based category (e.g. aviation)

```
1. Edit photos in Lightroom Classic
2. Export (see Lightroom settings below)
3. Open Cyberduck → navigate to (or create) the album folder:
      aviation/oshkosh-2024/
4. Upload all exported JPEGs
5. Upload a cover.jpg (your best shot from the album)
6. Optionally create and upload a manifest.json with title/date/location
...
**Primary workflow: Lightroom Classic on Mac.** Primary camera: Sony Alpha (ARW). Settings apply to any other camera Lightroom supports.

### Lightroom Classic export settings

File → Export → Export to Disk
...
**Display P3** makes a visible difference for saturated subjects — aircraft livery, jersey colors, arena lighting. Browsers on all modern Apple devices (and most Windows monitors made after ~2020) render the wider gamut. Older sRGB-only displays still render it correctly.

**Quality 90:** JPEG artifacts appear first in high-frequency detail — rivets, ice texture, bokeh edges. At 90 they are essentially invisible. Files land around 2–5 MB.

**Long edge 3000 px for both orientations:** Sufficient for 2× Retina at the photo page width (960 px) and the full-viewport lightbox.

**All Metadata:** Required for the build script to display camera body, lens, focal length, aperture, shutter speed, and ISO in the photo viewer. Sony Alpha ARW files carry full EXIF including lens data for native E-mount and A-mount lenses. If you use adapted lenses (e.g. via LA-EA5 or third-party adapters), lens name and focal length may not be embedded — some adapters do not pass that data through.

**Naming:** Any filename works. Album photo order is alphabetical, so Sony's default `DSC09999.jpg` scheme is fine for a single shoot. Be aware that Sony resets its counter and changes the prefix (DSC → \_DSC) when crossing 9999, which can split a shoot alphabetically — rename if that affects a single album.

### If you export from Lightroom Mobile
...
LR Mobile exports are sRGB (no Display P3 option) and lack output sharpening. To approximate sharpening: Detail → Amount 70–80, Radius 0.8–1.0, Masking 20–40, plus Texture +10 to +20.
...
Edit `CATEGORY_NAMES` in `scripts/build_catalog.py`. For example, to display a folder named `travel` as "Places":
```python
