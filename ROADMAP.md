...
## Phase 3 — Modern image pipeline (Python only; frontend already ready) ✅ COMPLETE
...
## Phase 4 — Social previews & SEO (build-time generation) ✅ COMPLETE
...
- [x] **Generate static per-album pages** at pretty canonical URLs
      (`/photography/<cat-slug>/<album-slug>/`) with real `og:`/Twitter/JSON-LD metadata,
      generated by `build_seo.py` and committed by the Action. Internal links + sitemap
      repointed to them; `album.html` now redirects old `?cat=&album=` links (and `#photo`
      hashes). `slugify` contract shared across `app.js`/`build_seo.py`/`album.html`.
      Finding #5. (Per-*photo* stubs still optional/future — `/photo/` sets og via JS.)
...
## Phase 5 — Presentation craft (design decisions, take slowly) ✅ COMPLETE
...
- [x] **Atom feed** of latest albums at `/feed.xml` (generated by `build_seo.py`,
      deterministic), advertised via `<link rel="alternate">` on the home + photography
      pages — fits the "Instagram alternative" goal.
- [x] **GPS / privacy** (finding #11): pipeline strips EXIF from *all* derivatives
      (thumbs/`_resized`/hero save without `exif=`) and the catalog records no GPS, so the
      only exposure is full-res originals. 4 locally-mirrored originals scanned clean;
      `scripts/check_gps.py` audits *all* R2 originals on demand (run with the `R2_*` env
      vars). Keep Lightroom's "Remove Location Info" enabled on export.
