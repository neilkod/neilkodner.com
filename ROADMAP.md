...
- [x] **Justified-row album grid** (vanilla flex; `gallery.js` sets `flex-grow`/`flex-basis`
      from each photo's aspect ratio) — preserves the curated left-to-right sequence. Finding #7.
- [x] **Self-host PhotoSwipe + Space Grotesk**: PhotoSwipe ESM+CSS vendored in `/vendor/`,
      Space Grotesk woff2 in `/fonts/` via `@font-face`; CSS `@import`s replaced with `<link>`
      tags (`tokens.css`) in every page `<head>`. Finding #10.
